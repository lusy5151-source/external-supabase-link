...
export default function PublicPlansList() {
  ...
  const fetchPublicPlans = useCallback(async () => {
    ...
    if (user) {
      ...
      ((apps as any[]) || []).forEach((a: any) => { appMap[a.plan_id] = a.status; });
      ...
      ((acceptedApps as any[]) || []).forEach((a: any) => {
      ...
      ((goingParts as any[]) || []).forEach((p: any) => {
  ...
  const handleApply = async () => {
    ...
    const { error } = await db
      .from("plan_applications")
      .insert({ plan_id: confirmPlan.id, user_id: user.id, status: "pending" });
    ...
    if (error) {
      ...
      await db.from("plan_notifications").insert({
  ...
  if (loading) {
    return <div className="flex items-center justify-center py-12"><p className="text-sm text-muted-foreground">공개 일정을 불러오는 중...</p></div>;
  }
            ...
            <div key={plan.id} className="rounded-2xl border border-border bg-card p-4 shadow-sm space-y-3">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm font-bold text-foreground cursor-pointer hover:text-primary transition-colors" onClick={() => navigate(`/plans/${plan.id}`)}>
              ...
              <div className="flex items-center gap-2">
                <Avatar className="h-6 w-6"><AvatarImage src={creator?.avatar_url || ""} /><AvatarFallback className="text-[9px]">{creator?.nickname?.[0] || "?"}</AvatarFallback></Avatar>
                <span className="text-xs text-muted-foreground">{creator?.nickname || "알 수 없음"}</span>
              </div>
              <div className="flex flex-wrap gap-3 text-[11px] text-muted-foreground">
                ...
                <span className="flex items-center gap-1"><Users className="h-3 w-3" />{currentCount}/{maxP}명</span>
              </div>
              {isCreator ? (
                <Button size="sm" variant="outline" className="w-full rounded-full text-xs" onClick={() => navigate(`/plans/${plan.id}`)}>내 계획 보기</Button>
              ) : myApp === "accepted" || myApp === "participant" ? (
                <Badge variant="secondary" className="w-full justify-center py-1.5 text-xs">참가 확정 ✅</Badge>
              ) : myApp === "pending" ? (
                <Badge variant="secondary" className="w-full justify-center py-1.5 text-xs bg-muted">신청 완료 (승인 대기)</Badge>
              ) : myApp === "rejected" ? (
                <Badge variant="secondary" className="w-full justify-center py-1.5 text-xs bg-destructive/10 text-destructive">신청 거절됨</Badge>
              ) : isFull ? (
                <Badge variant="secondary" className="w-full justify-center py-1.5 text-xs bg-muted">마감</Badge>
              ) : (
                <Button size="sm" className="w-full rounded-full text-xs bg-green-600 hover:bg-green-700 text-white" onClick={() => setConfirmPlan(plan)}>참가 신청</Button>
              )}
            </div>
          );
        })}
      ...
      <Dialog open={!!confirmPlan} onOpenChange={() => setConfirmPlan(null)}>
        <DialogContent className="rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {confirmPlan && mountains.find((m) => m.id === confirmPlan.mountain_id)?.nameKo} 등산 계획에 참가 신청할까요?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="flex gap-2">
            <Button variant="outline" onClick={() => setConfirmPlan(null)}>취소</Button>
            <Button onClick={handleApply} disabled={applying} className="bg-green-600 hover:bg-green-700 text-white">
              {applying ? "신청 중..." : "신청하기"}
            </Button>
          </DialogFooter>
        </DialogContent>
