    ...
    try {
      ...
      if (isNative) {
        ...
        const result = await SignInWithApple.authorize({
          clientId: "com.wandeung.app",
          redirectURI: "https://wandeung.com/auth/callback",
          scopes: "email name",
        });
      ...
      const message = getSupabaseErrorMessage(err, "Apple 로그인 처리 중 오류가 발생했습니다.");
      setAuthError(message);
      toast({ title: "인증 오류", description: message, variant: "destructive" });
    } finally {
      setLoading(false);
    ...
    <div className="flex min-h-[80vh] items-center justify-center pb-24">
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="space-y-2">
          <button onClick={handleAppleLogin} disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-xl border border-border bg-black px-4 py-3 text-sm font-medium text-white transition-colors hover:bg-gray-900 disabled:opacity-50">
